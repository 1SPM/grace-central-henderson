...
describe('property injection', () => {
  // CodeQL flagged writing a caller-supplied key onto a normal object
  // (js/remote-property-injection). The allowlist already rejected these, but
  // the result object now has a null prototype and the written key is a local
  // constant, so a future weakening of the check still cannot reach a
  // prototype.
  it.each(['__proto__', 'constructor', 'prototype'])('rejects %s as a section', (key) => {
    expect(validateSections({ [key]: ['x'] }, true).ok).toBe(false);
  });

  it.each(['__proto__', 'constructor', 'prototype'])('rejects %s as a segment', (key) => {
    expect(validateSegments({ [key]: 'en' }).ok).toBe(false);
  });

  it('returns objects with no prototype to pollute', () => {
    const sections = validateSections({ church: ['ok'] }, false);
    const segments = validateSegments({ language_preference: 'en' });
    expect(sections.ok && Object.getPrototypeOf(sections.value)).toBeNull();
    expect(segments.ok && Object.getPrototypeOf(segments.value)).toBeNull();
  });

  it('leaves Object.prototype untouched after a hostile payload', () => {
    validateSections(JSON.parse('{"__proto__":{"polluted":"yes"}}'), true);
    validateSegments(JSON.parse('{"__proto__":{"polluted":"yes"}}'));
