...
(function (global) {
  ...
  function c(variant, role) {
    ...
    if (role === 'navy') return inv ? '#fff' : ACCENT;
    if (role === 'gold') return LIGHT;
    if (role === 'nFill') return inv ? 'rgba(255,255,255,.18)' : ACCENT;
    if (role === 'gFill') return inv ? 'rgba(238,43,55,.35)' : LIGHT;
    return ACCENT;
  }
