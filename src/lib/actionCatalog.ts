 ...
 *
 * WHY THIS LIVES UNDER src/, NOT api/_lib/
 *
 * It used to live at api/_lib/actionCatalog.ts, re-exported the other
 * direction. Under `vercel dev`, the entire /api/ path is a reserved
 * namespace for serverless functions — there is no config-level way to
 * carve out a static-file exception, confirmed against Vercel's own
 * guidance. Since GraceChatContext.tsx imports this file at the top level,
 * that 404 broke the whole app from rendering locally. api/_lib/actionCatalog.ts
 * is now the thin re-export (mirrors ministryAreas.ts's existing pattern,
 * just in the correct direction for a file the browser needs at parse time).
 */
