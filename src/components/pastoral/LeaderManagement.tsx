...
const STATUS_CONFIG: Record<LeaderApplicationStatus, { label: string; color: string; icon: typeof Clock }> = {
  ...
  interview: { label: 'Interview', color: 'bg-slate-100 text-slate-700 dark:bg-slate-500/10 dark:text-slate-400', icon: Users },
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-4 py-2.5 bg-slate-600 hover:bg-slate-700 text-white rounded-xl text-sm font-medium transition-colors flex items-center gap-2"
        >
          <UserPlus size={16} />
          New Application
        </button>
        ...
        <div className="space-y-4">
          ...
          {filteredApps.length === 0 && (
            <div className="text-center py-12 text-gray-500 dark:text-dark-400">
              ...
              <button
                onClick={() => setShowOnboarding(true)}
                className="mt-3 px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white rounded-lg text-sm font-medium transition-colors"
              >
