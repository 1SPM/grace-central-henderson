            ...
            {serviceCards.map((card) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-900 dark:text-dark-100 mb-1 group-hover:text-slate-600 dark:group-hover:text-slate-400 transition-colors">
                    {card.title}
                  </h3>
          ...
          <div className="grid grid-cols-5 gap-4 overflow-x-auto pb-4">
            {pipelineStages.filter(s => s.status !== 'completed').map((stage) => {
              ...
              const colorClass = {
                ...
                violet: 'bg-slate-100 dark:bg-slate-900/30 text-slate-700 dark:text-slate-300',
                    ...
                    <tr key={request.id} className="hover:bg-gray-50 dark:hover:bg-dark-800/50 cursor-pointer">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          stage?.color === 'violet' ? 'bg-slate-100 dark:bg-slate-900/30 text-slate-700 dark:text-slate-300' :
