/**
 * Card Program — admin section of the Financial Hub (Phase C).
 *
 * Central Henderson is the issuer/operator: KYC approval queue,
 * issued-card roster (freeze/cancel + limits), and interchange
 * revenue rollup. All data flows through /api/neobank; in mock mode a
 * "simulate transaction" control exercises the real webhook pipeline
...
async function simulateTransaction(card: CardRecord): Promise<void> {
  ...
  const amountMicroUsd = (Math.floor(Math.random() * 9000) + 500) * 10_000; // $5–$95
  ...
  // Capture (member spend) + the interchange fee it generates for the church.
  ...
  await sendEvent({
    ...
    event_type: 'fee',
    i2c_card_id: card.i2c_card_id,
    amount_micro_usd: Math.max(10_000, Math.round(amountMicroUsd * 0.015)), // ~1.5% interchange
    direction: 'credit',
    merchant_name: merchant.name,
    occurred_at: new Date().toISOString(),
  });
...
export function CardProgramSection() {
  const [data, setData] = useState<AdminCardData | null>(null);
  const [state, setState] = useState<'loading' | 'ready' | 'unavailable' | 'gated'>('loading');
    ...
    <div className="mt-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: 'Interchange (MTD)', value: fmtUsd(data.summary.interchange_mtd_micro_usd), icon: TrendingUp, accent: 'text-emerald-600 dark:text-emerald-400' },
          { label: 'Card spend (MTD)', value: fmtUsd(data.summary.spend_mtd_micro_usd), icon: Zap, accent: 'text-slate-600 dark:text-slate-400' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-stone-100 dark:bg-dark-800 border border-stone-200 dark:border-dark-700 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-stone-200 dark:border-dark-700">
            ...
            <p className="text-xs text-gray-500 dark:text-dark-400">{liveCards.length} live</p>
          </div>
          <div className="divide-y divide-stone-200 dark:divide-dark-700 max-h-80 overflow-y-auto">
            {liveCards.length === 0 ? (
                ...
                <div key={card.id} className="p-3 flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 dark:text-dark-500 capitalize">
                      {card.status} · {fmtUsd(card.daily_limit_micro_usd)}/day
                    </p>
                  </div>
                  {data.adapter_mode === 'mock' && card.status === 'active' && (
                    <button
                      ...
                      title="Simulate a card transaction through the webhook pipeline"
            ...
            {data.interchange_events.slice(0, 25).map(e => (
              <div key={e.id} className="px-4 py-2.5 flex items-center gap-3 text-sm">
                ...
                <span className={`font-semibold ${e.direction === 'credit' ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-900 dark:text-dark-100'}`}>
                  {e.direction === 'credit' ? '+' : '−'}{fmtUsd(e.amount_micro_usd)}
                </span>
              </div>
            ))}
          </div>
...
function SectionHeader({ mode }: { mode?: 'live' | 'mock' }) {
  return (
    <div className="flex items-center justify-between mb-4">
      <div>
        <h2 className="serif text-2xl text-slate-900 dark:text-dark-100 leading-none">Card Program</h2>
        <p className="text-sm text-gray-500 dark:text-dark-400 mt-1">
          GRACE Impact Card — KYC, card roster, and interchange revenue
        </p>
      </div>
      {mode === 'mock' && (
