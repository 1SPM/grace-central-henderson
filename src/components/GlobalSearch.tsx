...
const NAV_ITEMS: { view: View; label: string; subtitle: string; icon: React.ReactNode }[] = [
  ...
  { view: 'sunday-prep', label: 'Sunday', subtitle: 'Prep & calendar', icon: <Church size={16} /> },
...
If you can't help directly, suggest what you CAN do: draft messages, find people, suggest follow-ups.`;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-50 pt-16 px-4" onClick={onClose}>
      <div className="bg-stone-100 dark:bg-dark-850 rounded-2xl w-full max-w-xl shadow-2xl flex flex-col max-h-[70vh]" onClick={(e) => e.stopPropagation()}>
        {/* Mode Tabs */}
        <div className="flex border-b border-gray-200 dark:border-dark-700">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition-colors ${
              mode === 'search'
                ? 'text-slate-600 dark:text-slate-400 border-b-2 border-slate-600 dark:border-slate-400'
            ...
            <form onSubmit={handleSubmit} className="p-3 border-t border-gray-200 dark:border-dark-700">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 bg-gray-100 dark:bg-dark-800 text-gray-900 dark:text-dark-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 placeholder:text-gray-400"
                  disabled={isAiLoading}
                />
                <button
                  type="submit"
                  disabled={!query.trim() || isAiLoading}
                  className="p-2.5 bg-slate-600 text-white rounded-xl hover:bg-slate-700 disabled:opacity-50 transition-colors"
                >
