/**
 * Puts Fixture #004's EvalCases under the normal `npx vitest run` gate,
...
import { FIXTURE_004_CASES } from './fixture-004-pastoral-care.cases.js';
import type { EvalResult } from '../types.js';

let results: EvalResult[];

beforeAll(async () => {
  results = await runCases(FIXTURE_004_CASES);
  const outDir = join(process.cwd(), 'tools/eval-harness/.output');
  mkdirSync(outDir, { recursive: true });
  writeFileSync(join(outDir, 'fixture-004-pastoral-care.json'), toJSON(results));
});

describe('Fixture #004 (pastoral care) via the eval harness', () => {
  ...
  it('the chat-door-bypass case is flagged as an architectural finding, not a plain pass', () => {
    const finding = results.find(r => r.id === 'pc-act-chat-door-bypasses-server-pipeline');
