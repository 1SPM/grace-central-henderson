/**
 * Puts Fixture #006's EvalCases under the normal `npx vitest run` gate,
...
import { FIXTURE_006_CASES } from './fixture-006-staff-work.cases.js';
import type { EvalResult } from '../types.js';

let results: EvalResult[];

beforeAll(async () => {
  results = await runCases(FIXTURE_006_CASES);
  const outDir = join(process.cwd(), 'tools/eval-harness/.output');
  mkdirSync(outDir, { recursive: true });
  writeFileSync(join(outDir, 'fixture-006-staff-work.json'), toJSON(results));
});

describe('Fixture #006 (staff/work) via the eval harness', () => {
  ...
  it('the chat-door case is flagged as an architectural finding, not a plain pass', () => {
    const finding = results.find(r => r.id === 'sw-act-add-and-update-are-chat-door-only');
