...
const routes=[];w.enterApp=()=>{};w.showScreen=route=>routes.push(route);w.history.replaceState({},'','?tour=1#profile');w.eval(fs.readFileSync('apps/member-web/public/tenants/faithful/faithful-mobile-tour.js','utf8'));assert.deepEqual(routes,['profile']);w.history.replaceState({},'','?tour=1#survey');w.dispatchEvent(new w.HashChangeEvent('hashchange'));assert.deepEqual(routes,['profile','survey'],'the pilot survey has a direct link facilitators can hand out');w.history.replaceState({},'','?tour=1#settings');w.dispatchEvent(new w.HashChangeEvent('hashchange'));assert.deepEqual(routes,['profile','survey'],'and anything not on the allowlist still goes nowhere');
