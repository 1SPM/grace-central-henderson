...
(function (global) {
   ...
   *  to. ('Central Henderson' → 'central-henderson' keeps existing data.) */
  function tenantSlug() {
  ...
  const memberDialogue = global.GRACE_MEMBER_INTENTS?.createSession();
  function think(text) {
    const t = (text || '').trim();
    ...
    if (RX.crisis.test(lower) || /immediate danger/i.test(lower)) {
      return {
    ...
    if (RX.whoAreYou.test(lower)) {
      return {
        intent: 'about',
        text: 'GRACE · AI assistant for ' + M.churchName + '. Find services, groups, events, and resources. For personal support, request human follow-up through pastoral care. Leader avatars are AI, not live pastors.'
      };
    }
    ...
    if (base.text === M.system.defaultResponse && t.replace(/[^a-z0-9]/gi, '').length > 2) {
      ...
      return {
        intent: 'unknown',
        text: 'I don’t have enough information to answer that yet. What would you like help finding?'
      };
    }
  ...
  function buildGreeting(page) {
    ...
    let text;
    if (d.visits <= 1) {
      text = 'GRACE · AI assistant. Hi ' + name() + '. What would you like to find at ' + M.churchName + '? ';
    } else {
      const fav = Memory.favouriteHourLabel();
  ...
  function micToggle() {
    ...
    if (voiceStatus) { voiceStatus.setAttribute('role','status'); voiceStatus.textContent = 'Starting voice input…'; }
    Listen.start(
      (text, isFinal) => {
        const inp = q('#gcp-input');
  ...
  const api = {
    ...
    ask(text) {
      ...
      const useLocalEngine = () => {
        setTimeout(() => {
          if(requestIdentity!==(global.Clerk?.user?.id||resolvedMemberId||A.demoMemberId||'anonymous')){memberDialogue?.reset();removeTyping();thinking=false;return;}
          ...
          if (resp.intent === 'unknown' || (A.quietNavigator && resp.preferService && !resp.navigateNow)) {
            ...
            askAi(text)
              .then((aiText) => {
                if(requestIdentity!==(global.Clerk?.user?.id||resolvedMemberId||A.demoMemberId||'anonymous')){memberDialogue?.reset();threadHistory.length=0;removeTyping();thinking=false;return;}
                removeTyping();
                thinking = false;
          ...
          pushHistory('model', resp.text);
          appendGrace(resp.text, resp.nav, resp.navLabel);
          if(resp.navigateNow && resp.nav)doNavigate(resp.nav);
      ...
      if (global.GRACE_SESSION) {
        askRealAssistant(text).then((result) => {
          ...
          if(requestIdentity!==(global.Clerk?.user?.id||resolvedMemberId||A.demoMemberId||'anonymous')){
    ...
    narratePage(text, onStart, onEnd) {
      if (!A || !Memory.data || thinking) return false;
