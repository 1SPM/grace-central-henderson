...
import { supabase, setClerkTokenProvider } from '@grace/platform-core/supabase';
import { resolveAuthMode } from './authMode';
import { TEMP_DISPLAY_NAME } from '../lib/greeting';
import { isDemoModeActive, TENANT_CHURCH_ID } from '@grace/platform-core/tenant';
...
function AuthProviderInner({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    void (async () => {
      const { setSentryUser } = await import('@grace/platform-core/observability/sentry');
      const { identifyUser, resetUser } = await import('@grace/platform-core/observability/posthog');
      if (user) {
