...
export function SetupChecklist({
  ...
  // Compact (sidebar) mode: one slim row naming the next incomplete step,
  // not the full list with six struck-through "done" rows underneath it —
  // that read as clutter in a narrow rail where the title itself already
  // truncates to "Finish Sett…".
  if (compact) {
    return (
      <div className="bg-stone-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 overflow-hidden shadow-sm">
        <button
          onClick={() => nextItem && onNavigate(nextItem.view)}
          className="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-gray-50 dark:hover:bg-dark-750 transition-colors"
        >
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-xs font-semibold text-gray-900 dark:text-dark-100 truncate">Setup {completedCount}/{items.length}</h3>
            {nextItem && (
              <p className="text-[10px] text-gray-500 dark:text-dark-400 truncate">{nextItem.label}</p>
            )}
          </div>
          <ChevronRight size={14} className="text-gray-300 dark:text-dark-600 flex-shrink-0" />
          <button
            onClick={(e) => { e.stopPropagation(); onDismiss(); }}
            className="p-1 hover:bg-gray-100 dark:hover:bg-dark-700 rounded-md transition-colors flex-shrink-0"
  ...
  return (
    <div className="bg-stone-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 overflow-hidden shadow-sm mb-6">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-gray-100 dark:border-dark-700/50 p-4">
      ...
      <div className="divide-y divide-gray-50 dark:divide-dark-700/30">
        {items.map(item => (
          <button
            ...
            className={`w-full flex items-center gap-2.5 text-left transition-colors px-4 py-2.5 ${
              item.done
                ? 'opacity-60'
