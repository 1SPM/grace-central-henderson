...
describe('watchCatalog', () => {
  it('every sermon has a non-empty thumbnail path', () => {
    for (const entry of WATCH_CATALOG) {
      ...
      expect(url).toMatch(/^\/watch\/.+\.jpg$/);
    }
  });

  it('known titles map to existing files under apps/admin-web/public/watch/', () => {
    const publicDir = join(import.meta.dirname, '../../public');
    for (const entry of WATCH_CATALOG) {
      const filePath = join(publicDir, 'watch', entry.thumbnailFile);
