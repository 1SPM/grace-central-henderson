...
import {
  ...
  churchShortName,
} from './tenants/centralHenderson';
import {
  FAITHFUL_CHURCH_DEFAULT_SETTINGS,
  FAITHFUL_CHURCH_GRACE_FACTS,
  FAITHFUL_CHURCH_TIMEZONE,
} from './tenants/faithfulChurch';

// Moved from src/hooks/useChurchSettings.ts (Phase 1 platform-core
// extraction) — this is the shape of a church's settings, needed by both
// the admin CRM (which manages it) and the Member Portal (which reads it),
// so it belongs here rather than in an admin-only hook. useChurchSettings.ts
// re-exports these for existing admin call sites.
