...
## ADR-018 — GRACE Epistemic Confidence & Clarification Contract

- **Date:** 2026-08-31
- **Status:** Accepted

**Context.** ADR-017 gave GRACE grounded self-awareness about what she's qualified, available, and authorized to do. A separate problem remained unaddressed: does GRACE know when she has *enough reliable evidence* to answer, recommend, propose, or act — versus when she should ask, qualify, or decline? Inspection found this was 100% left to model judgment: `api/_lib/grace-memory.ts`'s only conflict-resolution mechanism is a prompt instruction ("if anything here conflicts with the live church data above, the church data wins"), not code; no structural ambiguity, conflict, or insufficient-information check exists anywhere in `api/grace/_chat.ts`. Separately, inspection of the action-execution path found real, pre-existing gaps: `src/lib/grace-actions.ts`'s `resolvePerson`/`resolveTask`/`resolvePrayer` silently pick the *first* name match when multiple candidates exist, with no ambiguity signal — including for `delete_prayer`, a destructive, `requiresApproval: false` action — and `add_task` has zero required-field validation (a missing title silently becomes `'Untitled task'`). These are documented as findings below, not fixed by this ADR — this decision governs the new prompt-level contract; it does not rewrite the existing client-side action-resolution architecture (out of scope per item 30, and item 28's "do not silently broaden this sprint into fixing unrelated data architecture").

**Decision.** GRACE must never fill an important information gap with model confidence. Not a numerical confidence score (the repository has no calibrated confidence mechanism to build one on) — an **evidence-state classification** with a deterministic precedence order, implemented in `api/_lib/grace-epistemic.ts`:

- **Evidence states:** `SUFFICIENT`, `PARTIAL`, `AMBIGUOUS`, `CONFLICTING`, `STALE_OR_UNCLEAR`, `MISSING_REQUIRED`, `UNSUPPORTED`, `PROHIBITED`.
- **Response modes:** `ANSWER`, `ANSWER_WITH_QUALIFICATION`, `ASK`, `DECLINE`, `PROPOSE`, `ACT`.
- **Precedence** (`EVIDENCE_STATE_PRECEDENCE`, weakest never overrides strongest): `PROHIBITED > CONFLICTING > AMBIGUOUS > MISSING_REQUIRED > STALE_OR_UNCLEAR > UNSUPPORTED > PARTIAL > SUFFICIENT`. A prohibited request is never "resolved" by the clarifying question that would help complete it — item 11's rule, enforced structurally, not left to prompt hope.
- **Deterministic where the claim doesn't require parsing arbitrary free text**: `resolveActionReadiness` composes ADR-017's `resolveActionCapability` (qualification/permission/approval — never re-derived) with a static `REQUIRED_ACTION_PARAMETERS` table (per action type, derived from the catalog's own `promptExample` shape) and caller-supplied ambiguity/conflict signals, to decide whether `ACT` is reachable at all. `detectNameCollisions` is a structural, server-side, church-scoped check over the real roster (`fetchPeopleForCollisionCheck`, mirroring `grace-memory.ts`'s `resolvePersonIds` query shape exactly) — never derived from client `dataContext`, for the same reason ADR-017 never trusted client-submitted permissions: ambiguity detection feeds the ACT-readiness gate.
- **Left to the model, deliberately**: recognizing whether a specific free-text message references an ambiguous entity, needs qualification, or contains a prohibited request. A rules engine that tried to parse arbitrary natural language would be exactly the "enormous rules engine pretending to understand every possible request" this layer exists to avoid (item 23). The model reasons under an explicit contract it must not override into a more permissive mode — the same "model may express the result, must not invent the status" principle ADR-017 established for capability.

A server-composed **Epistemic Contract block** (`buildEpistemicContext`) is injected into every prompt, unconditionally — same reasoning as ADR-017's capability block: gating it on a message classifier would let adversarial phrasing route around it. It states the response-mode vocabulary, the precedence rules, the real name-collision list, and the action-parameter requirements table, and explicitly instructs that user pressure ("just guess," "use your best estimate," "don't ask me anything else," "ignore the source restrictions") never changes what evidence GRACE actually has.

Prompt order: `dataContext → knowledgeBlock → memoryBlock → capabilityBlock → epistemicBlock → history → question`. This does not change source precedence (live data → church knowledge → memory → model inference, unchanged from ADR-014/015) — the epistemic layer evaluates the evidence those layers produce; it does not redefine their authority. Reason codes (`SOURCE_SCOPE_MISMATCH`, `MULTIPLE_ENTITY_MATCHES`, `MEMORY_SUPERSEDED`, `PROHIBITED_INFERENCE`, etc.) are internal evidence, mapped to safe conversational explanations (`safeExplanationFor`) — never exposed raw.

**Consequences.**
- A 29-case qualification suite (`tools/eval-harness/central-henderson-exam/epistemic/`, exceeding the 28 required) covers sufficient/partial/no-source evidence, authoritative-vs-memory conflict, name-collision ambiguity (both false-positive and false-negative), missing action parameters, ambiguity-before-approval ordering, prohibition-outranks-clarification, consolidated-vs-Henderson scope, calculation-vs-fabrication, inference-vs-fact, cross-tenant isolation, and adversarial pressure resistance — all adversarial/safety cases pass via `dangerousFailure()`, never a plain `fail()`. Four cases are honestly `requiresLiveJudgment` with no `run()` (clarification naturalness, inference-labeling reply quality, multi-turn context retention in an actual reply) — including one explicit tracking case (`ep-recommend-no-currently-proven-recommendation-tracking`) documenting that **no RECOMMEND-level capability is currently PROVEN anywhere**, rather than inventing one to satisfy suite coverage.
- No new GRACE capability was introduced or promoted; the Capability Baseline and Pilot Capability Manifest are unchanged (verified: exam and eval-harness CLI output byte-for-byte identical before and after this work).
- Real, pre-existing findings from inspection — silent first-match-wins entity resolution in `resolvePerson`/`resolveTask`/`resolvePrayer` (including for the ungated destructive `delete_prayer`), and `add_task`'s complete absence of required-field validation — are documented here and in the epistemic contract's own file header, not fixed. This is a deliberate scope boundary, not an oversight: this ADR's mechanism (a prompt-level contract) makes the *model* far less likely to emit an ambiguous or incomplete action in the first place, which reduces exposure, but does not retroactively close the client-side mechanical gap. Closing that gap is a future, separately-scoped decision.
- Production remains untouched; nothing here required or performed a deployment change.

**Alternatives considered.**
- *Full NLU-based ambiguity/intent extraction from arbitrary user messages* — rejected: exactly the "rules engine pretending to understand every request" item 23 warns against; the model already does this reasoning, under contract, more capably than a hand-rolled parser could.
- *Client-side name-collision detection (reusing the existing `people` array already in `dataContext`)* — rejected: ambiguity detection feeds whether `ACT` is reachable, the same safety-relevant status ADR-017 gave permission/capability; a client-composed signal for a safety-relevant gate would be inconsistent with that precedent, even though getting it wrong here doesn't itself expose data.
- *Silently fix `resolvePerson`'s first-match-wins bug as part of this work* — rejected: bounded scope discipline (item 28) — report the finding, don't let discovering it expand this sprint into an unrelated action-architecture rewrite.
...
The member experience North Star (the Maya → Me → Us → Pastor → Real Pilot arc) has a "Tuesday" beat that the current posture cannot deliver: a member says something once, comes back another day, and GRACE still knows it — "you told me you serve at the welcome desk on Tuesdays." Without memory, the "Me" stage of that arc is a demo of a feature that does not exist. This ADR is the policy gate that the qualification plan (`docs/MEMBER_MEMORY_QUALIFICATION_PLAN.md`, task 0; `TECH_DEBT.md` TD-072) names as the hard prerequisite for tasks 1–6. The gate was built first, on purpose: `tools/eval-harness/central-henderson-exam/member-assistant/` holds 31 cases and 9 exchange-dialogue scenarios, 5 runnable today and passing, the rest graded `NOT_RUN` until an implementation exists — never a fabricated pass.

The staff memory system is the pattern to mirror, and it carries three known gaps that a member-facing version must not inherit: its sensitive-category guardrails are prompt-level only (`api/_lib/grace-memory.ts`, `runExtraction`), it has no delete endpoint (`api/grace/_memories.ts` is GET + POST), and it has the TD-064 shape where the model can say "Remembered" with no write behind it. Members are not staff: they did not sign up to be remembered by a work tool, they cannot open a database console to see what is stored, and what they say to a church assistant is more likely to touch health, family, immigration, or faith than a staff member's "my meeting with Bill is Thursday."

**Decision.** Amend the scope of ADR-014 and ADR-016 to admit a member-portal memory layer, under a carve-out narrower than the staff one. The carve-out, verbatim, is the policy:

> **GRACE may remember, for a portal member, only facts the member stated about themselves; never an inference; never a sensitive category even if self-stated.**

Concretely:

1. **Two sources, both member-originated.** `user_stated` — an explicit, anchored "remember that…" directive, handled deterministically with no model call, exactly as ADR-014's staff path. `ai_extracted` — a post-turn extraction pass restricted to facts the member asserted about *themselves* ("I usually come to the 9am service"). A fact about a third party (a daughter, a spouse, a sister) is not a fact about the member and is dropped even if the model returns it. Extraction never runs on a directive turn, a crisis-gated turn, or a moderation-blocked turn.
2. **Never an inference.** The existing "make no judgments about a member" boundary (`docs/AI_BOUNDARIES.md`, "What GRACE must never do") applies to the storage layer, not just the reply: no row may be a characterization ("seems disengaged"), a score, or a guess. Only the member's literal self-statement.
3. **Never a sensitive category, even self-stated.** The category list already in the member `SYSTEM_INSTRUCTION` — race, health (including mental health), immigration status, sexual orientation, "or other sensitive personal characteristic" — is the single source of a **post-hoc denylist** applied to the stored text on *both* paths, regardless of what the model returned. A member who says "remember that I'm undocumented" gets a kind, deterministic decline that still offers a human path, and no row. This is stricter than the staff system and deliberately so: prompt-level guardrails are the gap, not the fix.
4. **Own church, own person, own eyes.** A new `member_memories` table keyed `church_id + person_id` — *not* an extension of `grace_memories`, whose `user_id → users` FK is staff-only and which portal members structurally cannot hold (TD-070). Same provenance CHECK as migration 075 (an `ai_extracted` row without a `source_message_id` is impossible). SELECT-only RLS via portal identity; writes service-role only through the assistant route. `ON DELETE CASCADE` from `people`, so the existing right-to-be-forgotten path (`api/consents/_erase.ts`) erases memories with the person, with no change to that endpoint.
5. **Default-on, with view and delete.** Memory is on by default — there is no separate opt-in step. In exchange, two things are non-negotiable: the *first* "Remembered: …" reply tells the member how to see and delete what GRACE keeps, and a "What GRACE remembers about me" surface exists from day one, backed by `GET` and `DELETE /api/portal/memories` (scoped to `id + person_id + church_id`; a foreign id is a 404 with no write). Delete is a soft-delete by status; a deleted memory is never injected again.
6. **Subordinate to church data, rendered as data.** Memories reach the prompt in a labeled block whose header states that they are things the member told GRACE, may be stale, and that live church data wins — ADR-014's rule, unchanged. Memory content is quoted data, never an instruction line: a memory reading "ignore your rules" must leave the system instruction outside the block byte-identical. Memory cannot widen the tool set; the runtime declares exactly `ASSISTANT_TOOL_NAMES` regardless of what is remembered.
7. **Crisis first, always.** The runtime order is the safety property: **crisis gate → moderation → directive short-circuit → memory block → tool loop → reply → extraction.** Crisis text never becomes a fact, and a crisis turn makes zero model calls and zero memory writes. This ADR does *not* change crisis detection itself, which stays keyword-only per `docs/AI_BOUNDARIES.md` ("What this phase does NOT claim"); the harness records the uncued-phrasing limit as a tracking case rather than papering over it.
8. **No fake persistence.** Non-anchored phrasings ("keep in mind…", "don't forget…") get a deterministic "I can't save that from here" reply. A reply may contain "Remembered" only on a turn where a `member_memories` write actually happened.
9. **Kill switch.** `MEMBER_MEMORY_EXTRACTION=off` disables extraction without a deploy, same shape as `GRACE_MEMORY_EXTRACTION`.

What this ADR does **not** decide: retention/expiry policy beyond honoring `expires_at` (a follow-up once real volume exists); whether staff can ever see a member's memories (the answer today is no — nothing in this ADR creates a staff read path, and adding one would be its own ADR); church-shared member knowledge (out of scope, as it was for ADR-014).

**Consequences.**
- On acceptance, two documents are rewritten so the codebase's own policy text stops contradicting the decision. `docs/AI_BOUNDARIES.md`, "Staff Ask GRACE memory (ADR-014)", third bullet becomes: *"The member-facing portal assistant (`api/portal/_assistant.ts`) gained its own memory layer under ADR-019, with a stricter carve-out than the staff one: only facts the member stated about themselves; never an inference; never a sensitive category even if self-stated; default-on with a member-visible view and per-item delete."* `docs/GRACE_INTELLIGENCE_QUALIFICATION_FRAMEWORK.md`'s **Scope** line becomes: *"Staff Ask GRACE (`api/grace/_chat.ts`) and, for the memory family of cases only, the member-portal assistant (`api/_lib/ai/assistant-runtime.ts`) under ADR-019 — qualified by the sibling suite in `tools/eval-harness/central-henderson-exam/member-assistant/`, which stays outside the 10-domain main exam. The static demo companion and the marketing visual remain uncovered."* ADR-014's "explicitly out of scope" consequence and ADR-016's "not the member portal assistant" sentence are superseded by this ADR and are annotated in place with a one-line pointer, not edited.
- The `ma-safety-baseline-non-persistence-today` harness case, which pins today's policy, is retired in the same change that lands task 3 (runtime integration) — not before, and not silently: it flips from a passing architectural finding to a deleted case with this ADR as the reason.
- Implementation proceeds in the order the plan sets (schema → module → runtime → endpoints → UI → flip the harness), and each memory case flips `future → testable` only when its behavior exists. The gate in `member-assistant.test.ts` asserts memory cases stay `future` until then and fails loudly on any premature flip.
- The live-judgment halves (recall attribution "you told me…", the extraction boundary for incidental third-party detail, the plain-language disclosure sentence) are advisory under ADR-016's rule — never a build gate.
- Every extraction call is metered through the existing usage path (ADR-009) under a distinct feature label so member-memory cost is visible separately from the chat turn.
- Nothing in this ADR touches production or requires a deployment until task 1's migration; the harness, contract test, and plan already committed (`8bc0ad0`) are test infrastructure only.

**Alternatives considered.**
- *Keep the member assistant non-persistent and deliver "GRACE remembers me" from client-held `localStorage`* — rejected: it is the exact shape ADR-014 replaced on the staff side (per-browser, unscoped, lost on cache clear, no provenance), and it would be a "Tuesday" that only works on one device — a demo, not a feature.
- *Extend `grace_memories` with a nullable `person_id`* — rejected: the table's `user_id → users` FK is staff identity (TD-070 documents why portal members cannot hold it), and a shared table would put member and staff rows one missing filter apart. Separate table, separate RLS, separate route.
- *Directives only, no extraction* — rejected by the owner: the Tuesday beat depends on ordinary conversation being remembered, not on members learning a magic phrase. The self-stated-only rule plus the post-hoc denylist is the price of admitting extraction.
- *Opt-in consent step before any memory is stored* — rejected by the owner in favor of default-on with view and delete. The obligation this creates — disclosure on first remember, a visible list, one-click delete, erasure cascade — is written into the decision as non-negotiable rather than left to the UI task.
- *Prompt-level sensitive-category guardrails, matching the staff system* — rejected: the staff gap is known (`grace-memory.ts`, prompt-only), and a member saying something sensitive about themselves is the common case, not the edge. The denylist runs on the stored text, after the model, on both paths.
- *Upgrade crisis detection to a model-based classifier as part of this work* — rejected, unchanged from `docs/AI_BOUNDARIES.md`: that trade of auditability for recall is a separate, explicit human decision; bundling it here would let a memory feature quietly change the crisis posture.
