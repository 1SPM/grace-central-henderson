...
| 12 | **Approvals** (`PATCH /api/approvals`) | final go/no-go on gated actions | which actions are gated (catalog decides) | per church | `approvals.decide` only; proposer MAY approve own request (C-13) | `approvals` + `audit_logs` | per decision | **S** | no | **YES** |
