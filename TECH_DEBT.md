...
### TD-064 — Ask GRACE memory extraction silently loses almost every correction
- **Severity:** P1
...
- **Re-entry trigger:** before this checklist item is relied on for a demo, or the next time extraction-derived memory is used to justify the product's memory story to anyone outside this session.
- **Resolution path:** either raise `EXTRACTION_TIMEOUT_MS` with real timing data on how long the gateway-wrapped Claude call actually takes end-to-end in this environment, or stop awaiting extraction against a timeout at all — Vercel's `waitUntil()` (or an equivalent durable fire-and-forget) is the correct primitive for "finish this after the response is sent" in a serverless handler, which a bare `Promise.race` inside the handler's own execution isn't. Needs a deliberate call on which, not a reflexive timeout bump.

### TD-065 — Ask GRACE assistant-message writes can fail silently, losing the turn from history and extraction
- **Severity:** P2
...
- **Re-entry trigger:** any report of Ask GRACE "forgetting" something it just said, or before `grace_messages` is relied on as an audit-quality record of what Grace actually told a staff member.
- **Resolution path:** same class of fix as TD-060's documented pattern elsewhere in this file — check `error` on the insert, log it (at minimum to `security_events` or equivalent), and consider a bounded retry before giving up. Doesn't need to be transactional with the stream itself, just needs to stop failing silently.
