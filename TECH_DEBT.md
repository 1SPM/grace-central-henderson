...
- **Not fixed here because:** it is a policy decision first. Task 0 in `docs/MEMBER_MEMORY_QUALIFICATION_PLAN.md` is an ADR-019 amending ADR-014/016 with the carve-out "only facts the member stated about themselves; never an inference; never a sensitive category even if self-stated." Tasks 1–6 (schema, `api/_lib/member-memory.ts` with a post-hoc sensitive denylist, runtime order crisis → moderation → memory, `api/portal/_memories.ts` GET/DELETE, `grace-companion.js` UI, flipping the harness) follow it.
