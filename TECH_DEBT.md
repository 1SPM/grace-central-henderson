...
### TD-073 — The real member portal (`/portal`) has no inbound link; production serves the static preview instead
- **Severity:** P0 for pilot readiness (nothing built for `/portal` is reachable), P3 as a code defect (nothing is broken — the routing is exactly what it was deliberately configured to be, just never updated)
- **Location:** `apps/member-web/vercel.json` redirects the live domain's root to `members-card.html`; no file in `apps/member-web/public` links to `/portal`, which is served by `apps/member-web/src/portal/PortalRoot.tsx`.
- **Problem:** on 2026-09-03, during the member-portal audit ("Two Front Doors"), the decision was made that `/portal` — the real app: Clerk member auth, KYC, Stripe, a server-composed and moderated assistant with 14 tools — is the pilot surface, and the static tenant pages (`members-card.html`, `tenants/central-henderson/member-portal.html`, `tenants/faithful/member-portal.html`) demote to design spec and sales demo. That decision was never written into a tracked document (not `DECISIONS.md`, not `PILOT_READINESS.md`) and the routing was never changed. Re-verified live 2026-09-07: the redirect still points at `members-card.html`.
- **Why this matters more than it looks like a routing nit:** every feature built for `/portal` since that decision — the member-memory qualification gate (TD-072), the wallet work planned in TD-052's amendment — ships to a surface no member can currently reach. The gap isn't a missing feature; it's that the finished features have no door.
- **Not fixed here because:** flipping the redirect is a production cutover for the live Central Henderson tenant, not a demo tenant, and deserves a staged rollout (a test account or header-gated flag first), not a same-session change.
- **Resolution path:** `docs/PILOT_CRITICAL_PATH_PLAN.md`, Track A.
- **Re-entry trigger:** before any further work is planned "for `/portal`" as if it were already the front door — it is not, until this ships.
