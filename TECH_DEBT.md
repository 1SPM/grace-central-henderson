...
### TD-075 — Dependabot was bumping a phantom Stripe SDK while the real one sat eight majors behind — **unused dependency REMOVED; the real bump still outstanding**
- **Severity:** P2 for the stray dependency (removed here). The *finding* matters more than the fix: for as long as this stood, dependency automation reported Stripe as current while the SDK that actually moves money was untouched.
- **Found:** 2026-09-10, working PR #226 (`stripe` 20.1.2 → 22.6.1) as its own careful pass because it touches payments.
- **What was actually true.** All **18** Stripe consumers — payment intents, subscriptions, webhooks and their DLQ, Connect onboarding/status, portal sessions, checkout — live under `api/`, which declares its own `"stripe": "^14.10.0"` and resolves **14.25.0**. PR #226 changed only the root and `apps/admin-web` manifests, both of which declared `^20.1.2` and **neither of which imports the package at all**: the only `'stripe'` occurrences in `apps/admin-web/src` are a string literal in a credential-format validator, the app uses the browser SDKs (`@stripe/stripe-js`, `@stripe/react-stripe-js`), and the server SDK does not appear in the built bundle. **Merging #226 would have been a no-op for every code path that runs, while recording "Stripe is on 22.6.1" in the dependency history.** Closed unmerged, with that reasoning on the PR.
- **Fixed here:** `stripe` removed from `package.json` and `apps/admin-web/package.json`. It was inert — never imported, never bundled — but it is the **secret-key server SDK carried in a browser app's dependency list**: an unwatched supply-chain surface, and a trap for the next person whose editor autocompletes `import Stripe from 'stripe'` into frontend code. Verified after removal: lint, typecheck and build pass; the payment/webhook suites (`api/_lib/webhooks`, `api/billing`, `api/giving`, `api/webhooks`) pass **60/60**; `api/`'s own pin and installed 14.25.0 are untouched.
- **Still outstanding — the real work:** `api/` `stripe` **14.25.0 → 22.x**, eight majors. One thing in its favour: the wire API version is pinned explicitly in code (`apiVersion: '2023-10-16'`, `api/_lib/webhooks/stripe-dispatch.ts` and `api/_server.ts`) rather than inherited from the SDK default, so the risk is concentrated in TypeScript types and removed methods rather than silent API drift. Deserves its own pass with the webhook and payment suites run deliberately, and with it stated plainly that **no test in this repo exercises real Stripe** — the suites mock it.
- **Lesson:** "Dependabot has a PR open for it" is not evidence a dependency is current. Check which manifest actually feeds the code that runs — in a monorepo with a nested `api/` package, that is frequently not the one being bumped.
- **Re-entry trigger:** the `api/` v22 bump, or any Dependabot PR touching `stripe` — confirm it targets `api/package.json`, not the root.
